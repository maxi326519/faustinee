...
class Posts {
    ...
    public static function create($data) {
        ...
        $stmt = $pdo->prepare("INSERT INTO Posts (id, title, slug, category, contentHtml, coverUrl, tags, state, `reads`, author, date, fixedHome, fixedCategory, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$id, $data['title'], $data['slug'], $data['category'], $data['contentHtml'], $data['coverUrl'], $data['tags'], $data['state'], $data['reads'], $data['author'], $data['date'], $data['fixedHome'], $data['fixedCategory'], $now, $now]);
        return new self($id, $data['title'], $data['slug'], $data['category'], $data['contentHtml'], $data['coverUrl'], $data['tags'], $data['state'], $data['reads'], $data['author'], $data['date'], $data['fixedHome'], $data['fixedCategory'], $now, $now);
    }

    public static function update($id, $data) {
        $pdo = getPDO();
        $now = date('Y-m-d H:i:s');
        $stmt = $pdo->prepare("UPDATE Posts SET title = ?, slug = ?, category = ?, contentHtml = ?, coverUrl = ?, tags = ?, state = ?, `reads` = ?, author = ?, date = ?, fixedHome = ?, fixedCategory = ?, updated_at = ? WHERE id = ?");
        return $stmt->execute([$data['title'], $data['slug'], $data['category'], $data['contentHtml'], $data['coverUrl'], $data['tags'], $data['state'], $data['reads'], $data['author'], $data['date'], $data['fixedHome'], $data['fixedCategory'], $now, $id]);
    }
